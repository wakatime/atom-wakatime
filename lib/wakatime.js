...
export function activate() {
  ...
  options = new Options(getConfigFile());
...
function settingChangedHandler(settings, initial) {
  ...
  if (isValidApiKey(apiKey)) {
    ...
    options.setSetting('settings', 'api_key', apiKey);
  } else if (initial) {
    atom.config.set('wakatime.apikey', ''); // clear setting so it updates in UI
...
function setupConfigs() {
  options.getSetting('settings', 'api_key', (key) => {
...
function getLatestCliVersion(callback) {
  ...
  options.getSetting('settings', 'proxy', (proxy) => {
    options.getSetting('settings', 'no_ssl_verify', (noSSLVerify) => {
      options.getSetting('internal', 'cli_version_etag', (etag) => {
        options.getSetting('settings', 'alpha', (alpha) => {
          const opt = {
          ...
          if (etag.value) opt['headers']['If-None-Match'] = etag.value;
          try {
            request.get(opt, (error, response, json) => {
              if (!error && (response.statusCode == 200 || response.statusCode == 304)) {
                ...
                if (response.headers.etag) {
                  options.setSettings('internal', [
                    {key: 'cli_version', value: latestCliVersion},
                    {key: 'cli_version_etag', value: response.headers.etag},
                  ]);
                }
                callback(latestCliVersion);
                return;
...
function getResourcesLocation() {
  ...
  try {
    ...
    resourcesLocation = extensionFolder();
  }
...
function getCliLocation() {
  ...
  return path.join(getResourcesLocation(), `wakatime-cli-${platform}-${arch}${ext}`);
}
...
async function removeCLI() {
  try {
    await del([getResourcesLocation()], { force: true });
  } catch (e) {
    log.warn(e);
  }
}
...
function cliDownloadUrl(version) {
  ...
  if (!validCombinations.includes(`${platform}-${arch}`)) reportMissingPlatformSupport(platform, arch);

  return `${githubDownloadPrefix}/${version}/wakatime-cli-${platform}-${arch}.zip`;
}

function reportMissingPlatformSupport(platform, architecture) {
  const url = `https://api.wakatime.com/api/v1/cli-missing?platform=${platform}&architecture=${architecture}&plugin=atom`;
  let opt = {
    url: url,
