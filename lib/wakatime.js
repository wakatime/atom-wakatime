...
export function activate() {
  ...
  packageName = atom.getAppName().toLocaleLowerCase().replace(' ', '_');
...
function reportMissingPlatformSupport(osname, architecture) {
  const url = `https://api.wakatime.com/api/v1/cli-missing?osnameame=${osname}&architecture=${architecture}&plugin=${packageName}`;
  let opt = {
    url: url,
...
function sendHeartbeat(file, lineno, isWrite) {
  ...
  if (isWrite || enoughTimePassed(time) || lastFile !== currentFile) {
    const args = ['--entity', currentFile, '--plugin', `${packageName}-wakatime/${packageVersion}`];
    if (isWrite) {
      args.push('--write');
    }
    if (lineno != null) {
  ...
  try {
    const proc = child_process.execFile(binary, args, (error, stdout, stderr) => {
      if (error != null) {
        ...
        if (proc.exitCode === 102 || proc.exitCode === 112) {
          msg = null;
          status = null;
          title = 'WakaTime Offline, coding activity will sync when online.';
        } else if (proc.exitCode === 103) {
          msg = 'An error occured while parsing $WAKATIME_HOME/.wakatime.cfg. Check $WAKATIME_HOME/.wakatime/wakatime.log for more info.';
          status = 'Error';
          title = msg;
          ...
          msg = error;
          status = 'Error';
          title = `Unknown Error (${proc.exitCode}); Check your Dev Console and ~/.wakatime/wakatime.log for more info.`;
        }
...
function getToday() {
  ...
  const args = ['--today', '--plugin', `${packageName}-wakatime/${packageVersion}`];
  if (atom.config.get('wakatime.disableSSLCertVerify')) {
    args.push('--no-ssl-verify');
  }
  args.push('--config');
