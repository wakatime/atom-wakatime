...
export function activate() {
  ...
  options = new Options(getConfigFile(), getConfigFile(true));
...
function reportMissingPlatformSupport(osname, architecture) {
  const url = `https://api.wakatime.com/api/v1/cli-missing?osname=${osname}&architecture=${architecture}&plugin=atom`;
  let opt = {
    url: url,
