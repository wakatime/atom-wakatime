...
export function activate() {
    ...
    return requestIdleCallback(delayedActivate, {timeout: 10000});
}
...
export function deactivate() {
    ...
    return (settingChangedObserver != null ? settingChangedObserver.dispose() : undefined);
}
...
function checkCLI() {
  if (!isCLIInstalled()) {
    ...
    return getLastCheckedForUpdates(lastChecked => {
...
function setLastCheckedForUpdates(lastChecked) {
  const filePath = path.join(resourcesFolder(), 'last-checked-for-updates');
  return fs.writeFile(filePath, lastChecked.toString(), {encoding: 'utf-8'}, err => {
    if (err != null) {
...
function setupConfigs() {
  const configFile = path.join(getWakatimeHome(), '.wakatime.cfg');
  return fs.readFile(configFile, 'utf-8', (err, configContent) => {
    if (err != null) {
      log.debug('Error: could not read wakatime config file');
...
function installCLI(callback) {
  ...
  return downloadFile(url, zipFile, () => extractCLI(zipFile, () => {
...
function extractCLI(zipFile, callback) {
...
function removeCLI(callback) {
  if (fs.existsSync(path.join(resourcesFolder(), 'wakatime-cli'))) {
    try {
      return rimraf(path.join(resourcesFolder(), 'wakatime-cli'), () => {
        if (callback != null) {
          return callback();
        }
      });
...
function executeHeartbeatProcess(binary, args, tries) {
  ...
  try {
    let proc;
    return proc = child_process.execFile(binary, args, (error, stdout, stderr) => {
      if (error != null) {
...
function getToday() {
  ...
  try {
    let proc;
    return proc = child_process.execFile(cliLocation(), args, (error, stdout, stderr) => {
      if (error != null) {
        if ((stderr != null) && (stderr !== '')) {
          log.debug(stderr);
        }
