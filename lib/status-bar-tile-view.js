...
class StatusBarTileView extends HTMLElement {
  ...
  show() {
    ...
    this.classList.remove(element_name, 'hidden');
  }

  hide() {
    this.classList.remove(element_name, 'inline-block');
    this.classList.add(element_name, 'hidden');
  }
  ...
  setStatus(content, override_errors) {
    if (override_errors == null) { override_errors = true; }
    if (override_errors || /^error/i.test((this.msg != null ? this.msg.textContent : undefined) || '')) {
...
export default statusBarElm = document.registerElement(element_name, {prototype: StatusBarTileView.prototype, extends: 'div'});
