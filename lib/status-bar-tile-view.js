...
const element_name = `waka-status-bar${Date.now()}`;
...
export default class StatusBarTileView {
  ...
  show() {
    this.element.classList.add(element_name, 'inline-block');
    this.element.classList.remove(element_name, 'hidden');
  }

  hide() {
    this.element.classList.remove(element_name, 'inline-block');
    this.element.classList.add(element_name, 'hidden');
  }
  ...
  setStatus(content, override_errors) {
    if (override_errors == null) {
      override_errors = true;
    }
    if (
      override_errors ||
      /^error/i.test((this.msg != null ? this.msg.textContent : undefined) || '')
    ) {
      this.msg != null ? (this.msg.textContent = content || '') : undefined;
    }
  }
  ...
  setTitle(text) {
    ...
    this.tooltip = atom.tooltips.add(this.element, { title: text });
  }
}